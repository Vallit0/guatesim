...
% =====================================================================
\section{Discussion}
\textbf{Why the reversal matters.} The two models swap order between B1
(the optimum of the \emph{stated} reward) and B3 (the \emph{executed}
national budget): GPT-4o-mini scores ``safer'' on the deployer's own
brief yet sits farther from the country's actual fiscal process, while
Claude does the opposite. Alignment measured against a single reference
is therefore under-determined---a scalar ``safety score'' would hide the
reversal that the B1/B2/B3 triangulation exposes. This is the central
methodological claim: the two models are not better and worse, they are
\emph{differently} misaligned.

\textbf{Revealed vs.\ stated behavior.} Both models pass
instruction-following in prose, yet over eight quarters they concentrate
the recovered reward on \texttt{anti\_poverty}, zeroing the secondary
objectives the brief also requested. This revealed-preference
gap---invisible to capability or toxicity benchmarks---is precisely what
BRCA is built to surface, and it is why a budget-decision deployer should
audit \emph{choices}, not \emph{claims}.

\textbf{What a deployer should take away.} Operationally: (i) test the
revealed constitution, not the stated one; (ii) demand the reference
triangulation rather than a single number, since ``which model is
safer'' depends on the reference privileged; and (iii) require disclosure
of what is \emph{conditional}---the binary misalignment verdict is
robust, but the recovered direction depends on the prior width and the
feature scaling, which BRCA reports explicitly.
