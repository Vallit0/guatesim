...
% =====================================================================
\section{Method}
BRCA is a seven-layer pipeline (Fig.~\ref{fig:pipeline}) with two
phases (Fig.~\ref{fig:arch}): an \emph{online} rollout in which a
calibrated simulator and the LLM interact for $T$ turns, and an
\emph{offline}, black-box audit of the resulting trajectory.

\begin{figure*}[t]
\centering
\includegraphics[width=0.95\textwidth]{pipeline_7stage.pdf}
\caption{The seven typed layers of BRCA. The world simulator (Layer~1)
and the human-process anchor (B3) are country-specific; Layers~2--7 and
the normative baselines (B1,~B2) are country-agnostic. Each box lists
its output; the italic label beneath, its input.}
\label{fig:pipeline}
\end{figure*}
...
\textbf{Choice model and recovery.} At turn $t$ the menu offers $K{=}5$
candidate budgets over the nine-item simplex. We assume rational
Boltzmann choice \cite{ziebart2008,mcfadden1974} (Fig.~\ref{fig:choice}):
\begin{equation}
...
\label{eq:boltzmann}
\end{equation}
with temperature $\beta$ absorbed into $\|\boldsymbol{\theta}\|$. We place
$\boldsymbol{\theta}\sim\mathcal{N}(\mathbf{0},\sigma^2 I)$ and sample the
posterior with NUTS in PyMC \cite{ramachandran2007} (Fig.~\ref{fig:irl}).
The six features are signed ``higher is better'':
...
\caption{Stage~3 (real data). Five candidate budgets, the Boltzmann
choice probabilities $P(k)$ under $\boldsymbol{\theta}_{\text{rec}}$
(Eq.~\ref{eq:boltzmann}), the actual selection (outlined), and a real
...
\caption{Stage~4. Left: the generative model (prior
$\to\boldsymbol{\theta}\to$ Boltzmann choices, plated over $T$). Right:
the recovered posterior (Claude, 20 seeds); \texttt{anti\_poverty}
dominates.}
\label{fig:irl}
\end{figure}
...
\boldsymbol{\theta}_{\text{rec}})$, with a 95\% HDI and a
region-of-practical-equivalence (ROPE) of width $0.25$ per dimension
\cite{kruschke2013,hadfield2017} (Fig.~\ref{fig:ird}).

\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{fig_ird_radar.pdf}
\caption{Stage~5. Stated vs.\ recovered constitution over the six
dimensions ($L_1$-normalized). Both models collapse onto
\texttt{anti\_poverty}; cosine to stated is $0.70$ (Claude) / $0.72$
