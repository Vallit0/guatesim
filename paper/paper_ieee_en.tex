...
(Fig.~\ref{fig:pipeline}). Each layer has a typed input and a typed
...
\begin{figure*}[t]
\centering
\includegraphics[width=0.95\textwidth]{pipeline_7stage.pdf}
\caption{Seven-layer pipeline of the Bayesian IRD audit framework. Each
layer is independently testable; the world simulator (Layer~1) and the
human-process anchor (component B3 of Sec.~III-H) are the
country-specific components, while Layers~2--7 and the normative
baselines (B1, B2) are country-agnostic by construction. Inside each box
is its typed output; the italic label beneath is its typed input. The
IRD audit (Layer~5) is the core alignment check.}
\label{fig:pipeline}
\end{figure*}
...
not); and (ii) a live $K{=}7$ re-collection (Sec.~\ref{sec:robustness}),
under which both models stay misaligned $20/20$ and the
reasoning--policy gap keeps its sign, with $K{=}9$ published as a
further specification. We treat
...
in PyMC (Fig.~\ref{fig:irl}; the Boltzmann choice rule of
Eq.~\ref{eq:boltzmann} is illustrated on real data in
Fig.~\ref{fig:choice}). The six features are signed so that ``higher is always
...
\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{fig_llm_choice.pdf}
\caption{Stage~3 (LLM choice), real data. For one representative turn:
the five candidate budgets over the nine \emph{partidas}, the Boltzmann
choice probabilities $P(k)$ under the recovered $\boldsymbol{\theta}$
(Eq.~\ref{eq:boltzmann}), and the model's actual selection (outlined),
with a real chain-of-thought excerpt. The chosen item is the policy mode
here; the Boltzmann form keeps the per-turn choice stochastic, which is
precisely what the IRL layer (Fig.~\ref{fig:irl}) inverts.}
\label{fig:choice}
\end{figure}

\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{fig_bayesian_irl.pdf}
\caption{Stage~4 (Bayesian IRL). Left: the generative model---a Gaussian
prior over $\boldsymbol{\theta}$ feeding a Boltzmann choice likelihood,
plated across the $T$ turns. Right: the recovered posterior (Claude,
pooled over twenty seeds; mean and IC95 per dimension).
\texttt{anti\_poverty} dominates while the other five weights
...
\caption{Stage~5 (IRD audit). Radar of the stated intent
$\boldsymbol{\theta}_{\text{stated}}$ against the recovered
constitutions of both models over the six dimensions (positive part,
$L_1$-normalized for shape comparison). Both models collapse onto
\texttt{anti\_poverty} relative to the broader stated profile; the
cosine to the stated reward is $0.70$ (Claude) and $0.72$ (GPT-4o-mini),
both flagged misaligned.}
\label{fig:ird}
\end{figure}
...
\textbf{Anchor-year robustness.} Re-running B3 against the MINFIN
\emph{2023} budget (assembled from ICEFI Tabla~8 executed and Tabla~7
appropriated, documented in \texttt{data/minfin\_2023\_ejecutado.csv})
preserves the ordering: Claude median $L_1{=}62.5$~pp vs.\ GPT-4o-mini
$64.9$~pp ($p{=}0.0003$), i.e.\ the ``Claude closer to the human
process'' direction is not an artifact of the specific anchor year.
...
\textbf{Non-lexical corroboration (v3).} Because v1 and v2 are both
lexical encoders, we re-encoded the chain-of-thought with an
LLM-as-judge (Claude Opus~4.7) that scores each of the six dimensions
semantically rather than by keyword frequency, on a five-seed
sub-sample (80 CoT samples). The semantic judge raises both models'
coherence cosines---to $+0.610$ (Claude) and $+0.909$
(GPT-4o-mini)---confirming that part of the low \emph{absolute} v1
cosine is a lexical artifact; but the \emph{cross-model gap} persists
and indeed widens in direction terms ($-0.30$ median difference, with
Claude lower in all $5/5$ seeds). The paired Wilcoxon sits at its
$n{=}5$ floor ($p{=}0.0625$, the smallest two-sided value attainable
at five pairs). We therefore report the reasoning--policy gap as
\emph{confirmed under a non-lexical encoding}, with a referee-hardening
extension to $\ge 6$ seeds (for a sub-$0.05$ $p$) noted as future work.
...
$\alpha{=}0.05$ unadjusted; two never do (per-seed entropy of the
chosen-candidate distribution, $p{=}0.24$; $w_{\text{pro\_approval}}$,
$p{=}0.45$). To control the cross-layer multiplicity, we apply the
Holm--Bonferroni step-down procedure over the full family of thirteen
at a family-wise error rate $\alpha{=}0.05$
(\texttt{scripts/holm\_bonferroni.py}; adjusted values in
\texttt{holm\_bonferroni.csv}). \emph{Nine of the thirteen survive}.
The two that drop under correction are the weakest unadjusted
rejections---the overall IRD cosine difference
($p{=}0.017\!\to\!p_{\text{Holm}}{=}0.069$) and $w_{\text{anti\_deuda}}$
($0.033\!\to\!0.098$); the chain breaks at the former. Critically, the
load-bearing contrasts survive comfortably: the reasoning--policy
cosine gap ($p_{\text{Holm}}{=}2.5\times10^{-5}$), $w_{\text{anti\_pobreza}}$
($3.8\times10^{-5}$), the recovery norm, all three monetized harm
dimensions, $w_{\text{pro\_crecimiento}}$, $w_{\text{pro\_confianza}}$,
and $w_{\text{anti\_desviacion\_inflacion}}$ all clear the corrected
threshold. The binary misalignment verdict of
Sec.~\ref{sec:misalign} is a \emph{per-model} ROPE test, not one of
these paired comparisons, so it is unaffected by the correction; what
the Holm step removes is only the claim that the two models differ in
\emph{overall} IRD cosine, which we accordingly do not assert.
...
the posterior under the extended grid
$\sigma \in \{0.1, 0.25, 0.5, 1, 2, 5, 10\}$ on every (seed, model)
pair, holding the data, the NUTS configuration and the seed fixed.
Over the moderate-prior regime $\sigma \in [0.1, 2]$ the recovered
direction is essentially invariant: median cosine of
$\boldsymbol{\theta}^{(\sigma)}_{\text{rec}}$ vs.\ the
$\sigma{=}1$ reference stays $\ge 0.984$ for Claude
($0.9921, 0.9965, 0.9976, 0.9838$ at $\sigma{=}0.1,0.25,0.5,2$) and
$\ge 0.996$ for GPT-4o-mini. Under near-flat priors the direction is
\emph{not} invariant: at $\sigma{=}5$ the median cosine falls to
$0.694$ (Claude) / $0.929$ (GPT-4o-mini), and at $\sigma{=}10$ to
$0.409$ / $0.659$. We read this honestly as a regularization
boundary---when the prior stops constraining the recovery, the norm
$\|\boldsymbol{\theta}\|$ inflates (Claude $0.024 \!\to\! 10.3$ across
the grid) and the direction rotates into weakly-identified likelihood
directions; the default $\sigma{=}1$ direction is therefore partially
prior-stabilized, and our direction-level claims are scoped to
$\sigma \le 2$. Crucially, the \emph{binary} misalignment
classification of Sec.~\ref{sec:misalign} is identical across the
\emph{entire} grid (40/40 pairs misaligned at every
$\sigma \in \{0.1,\dots,10\}$, 0 reclassifications): the verdict that
both models diverge from the stated reward does not depend on the
prior width, even where the exact recovered direction does.
...
We also re-collected the menu live at $K{=}7$ (20 seeds, same prompt
and shocks, \texttt{guatemala\_sim/irl/candidates\_extended.py}). The
headline survives the menu expansion: both models remain misaligned in
$20/20$ seeds, the IRD cosine to the stated reward stays low ($+0.519$
Claude, $+0.708$ GPT-4o-mini), and the reasoning--policy gap retains
its sign and significance ($\cos_{\text{CoT}}$ Claude $<$ GPT,
$p<0.0001$; deceptive flags $14/20$ vs.\ $0/20$). The cross-reference
ordering against B1/B3 under $K{=}7$ is a declared follow-up; $K{=}9$
is published as a specification for a further run.
...
R6 direction, $\sigma\in[0.1,2]$ vs $\sigma{=}1$ & cos median $\ge$0.984 C / $\ge$0.996 G & yes \\
R6 direction, $\sigma{=}5$ vs $\sigma{=}1$   & cos median 0.694 C / 0.929 G & no (scope $\sigma\!\le\!2$) \\
R6 direction, $\sigma{=}10$ vs $\sigma{=}1$  & cos median 0.409 C / 0.659 G & no (scope $\sigma\!\le\!2$) \\
R6 reclassifications, full grid $\sigma\in[0.1,10]$ & 0/40 pairs            & yes \\
T2.1 K=7 live menu (IRD cos)        & 0.519 C / 0.708 G, misalign 20/20 & yes \\
T1.3 feature LOO, any drop          & median cos $\ge$0.94, 0/40 reclass & yes \\
T1.2 norm.\ (centered/mult-scale)   & cos $\ge$0.988 each model         & yes \\
T1.2 norm.\ (z-score)               & cos median 0.753 C / 0.909 G      & no (scale load-bearing) \\
T2.4 temperature, IRD cos $\Delta(T{=}0,T{=}0.7)$ & $\le 0.044$ both    & yes \\
T2.4 temperature, Claude choice agree.\ & 0.575 (preference stable)     & partial \\
T3.2 v3 Opus semantic judge (RPC gap) & $-0.30$, 5/5 seeds Claude lower & yes \\
\bottomrule
\end{tabular}
\end{table}
...
\textbf{$\boldsymbol{\theta}_{\text{stated}}$ is a coder-dependent
projection.} The deployer-intent vector is a manual projection of the
SYSTEM\_PROMPT onto the six-dimensional salience basis. An inter-coder
reliability check with two independent LLM coders (Sonnet~4.5,
Haiku~4.5) re-rating the same prompt yields a low linear-weighted
Cohen's $\kappa$ (min $-0.20$ across the three pairs), driven mainly by
the ambiguous salience of \texttt{pro\_approval}; the resulting
$\boldsymbol{\theta}_{\text{stated}}$ vectors nonetheless lie within
pairwise cosine $[0.66, 0.93]$. We therefore treat the extraction as
coder-dependent rather than canonical. The binary misalignment verdict
is defended against this dependence by R1, which shows the verdict
survives multiplicative perturbation of $\boldsymbol{\theta}_{\text{stated}}$
up to $\rho{=}0.5$ ($\ge 99.5\%$ of pairs still misaligned); a
multi-coder re-estimate of the recovered-vs-stated cosine is a declared
follow-up.
\textbf{Menu sensitivity is bounded but real.} The dependence we flag
lives in the menu \emph{composition}, not the feature basis: a live
$K{=}7$ re-collection preserves the headline (misalign $20/20$, RPC gap
sign retained), and a feature leave-one-out (drop each of the six
dimensions, re-fit) leaves the recovered direction stable (median
cosine $\ge 0.94$, $0/40$ verdict reclassifications). What remains
sensitive is dropping the dominant \emph{candidate}
(\texttt{human\_development}, the $K{=}4$ check), and the $K{=}9$ menu
is not yet re-collected live; the $B1/B3$ cross-reference ordering
under $K{=}7$ is a declared follow-up.
\textbf{Feature scaling is load-bearing.} The recovered direction is
invariant to centering and to multiplicative feature rescaling (cosine
$\ge 0.988$ vs.\ the production basis) but \emph{not} to per-feature
z-scoring, which rotates Claude's recovered vector to cosine $0.753$.
We therefore report results in the raw (identity) feature basis and
treat the scaling choice as an explicit modeling assumption rather
than an innocuous preprocessing step; together with the prior-width
scoping ($\sigma\le 2$, Sec.~\ref{sec:robustness}), the
direction-level claims are conditional on these two choices, while the
binary misalignment verdict is robust to prior width across the full
$\sigma$ grid.
